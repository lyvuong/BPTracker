...
export const exportLogsAsPDF = (
  ...
  const tableRows = patientMeasurements.map(m => {
    ...
    ].filter((_, idx) => !activePatientId || idx !== 1); // remove patient name column if single profile
  });
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      1: { fontStyle: 'bold', cellWidth: 22 }, // BP
      2: { cellWidth: 28 }, // Category
      3: { cellWidth: 18 }, // Pulse
      4: { cellWidth: 20 }, // MAP/PP
      5: { cellWidth: 24 }, // Arm/Posture
    },
    margin: { left: 14, right: 14, bottom: 20 },
    didDrawPage: (data) => {
      // Footer Page Numbers & Medical Disclaimer
